...
{{--    </nav>--}}
    ...
    <section class="projects-table">
        <div class="projects-container container">
            <h2 class="title">   {{languageGet()=='en'?'Order List':'لائحة الطلبات'}}</h2>
            <div class="table-wrapper">
                <table>
                    <thead>
                    <tr>
                        <th class="center"> {{languageGet()=='en'?'Order Id':'رقم تعريف الطلب الخاص'}}</th>
                        <th class="center"> {{languageGet()=='en'?'Date':'تاريخ'}}</th>
                        <th class="end"> {{languageGet()=='en'?'Amount':'كمية'}}</th>
                        <th class="center"> {{languageGet()=='en'?'Payment Status':'حالة السداد'}}</th>
                        <th class="center"> {{languageGet()=='en'?'Action':'فعل'}}</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($order_list as $order)
                        <tr>
                            <td class="center">#{{$order->invoice_id}}</td>
                            <td class="center">{{$order->date}}</td>
                            <td class="end">${{$order->total_payable}}</td>
                            <td class="center">
                                @if($order->payment_status==1)
                                    <span>   {{languageGet()=='en'?'Paid':'مدفوع'}}</span>

                                @else
                                    <span>  {{languageGet()=='en'?'Unpaid':'غير مدفوعة الأجر'}}</span>
                                @endif
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                        <li><a class="dropdown-item" href="{{route('user.my.order.details',['order_id'=>$order->id])}}">{{languageGet()=='en'?'Order Details':' تفاصيل الطلب'}}  </a></li>
                                    </ul>
                                </div>
                            </td>
