...
<section class="sign-up-area">
    <div class="container-alt set-bg spacing">
        ...
        <form action="{{route('admin.registration.store')}}" method="post" style="margin-top: -39px;">
                @if(Session::has('success'))
                    <h4 class="msg-success">{{ Session::get('success')}}</h4>
                @endif
...
{{--            </select>--}}
            ...
            <input required type="hidden" name="user_type" value="general_user" />
        ...
        <div class="sign-up-text">
            <p class="log-in-text">
                Don't have an account? <a href="{{route('frontend.user.login')}}">Log In</a>
            </p>
            <p>Or connect with</p>
        </div>
